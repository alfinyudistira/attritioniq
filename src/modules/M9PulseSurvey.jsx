...
function generatePulseHistory(src) {
  ...
  return weeks.map((week, wi) => {
    ...
    depts.forEach(dept => {
      ...
      deptData[dept] = {
        workload:    Math.max(1, Math.min(10, (avgSat * 0.90) - degradation * 0.30 + (rand() - 0.5))).toFixed(1),
        recognition: Math.max(1, Math.min(10, (avgSat * 0.85) - degradation * 0.20 + (rand() - 0.5))).toFixed(1),
        ...
        energy:      Math.max(1, Math.min(10, (avgSat * 0.90) - degradation * 0.40 + (rand() - 0.5))).toFixed(1),
        responses,
...
function ResponseHeatmap({ responses }) {
  ...
  const heatData = (() => {
    ...
    return days.map((_, di) => hours.map(h => {
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <div>
          <div style={{ fontSize: 12, fontWeight: 700, color: "#92400e" }}>
            Sample Data — {SAMPLE_DATA_METADATA.name}
          </div>
          <div style={{ fontSize: 11, color: "#b45309", marginTop: 1 }}>
            All charts and scores below are simulated from demo employees, not your real workforce. Upload your CSV to see real insights.
          </div>
        </div>
      </div>
      <button
...
function M9EmptyState({ onShowSample }) {
  return (
    <div style={{
      ...
      <div style={{ fontFamily: "'Playfair Display',Georgia,serif", fontSize: 22, fontWeight: 700, color: "#0f172a", marginBottom: 8 }}>
        Micro-Pulse Survey Engine
      </div>
      <div style={{ fontSize: 14, color: "#64748b", lineHeight: 1.7, marginBottom: 28, maxWidth: 380, margin: "0 auto 28px" }}>
        Generate weekly survey links, track real-time sentiment, catch burnout early warnings, and auto-sync satisfaction scores back to M1.
      </div>

      {/* Feature preview pills */}
      <div style={{ display: "flex", flexWrap: "wrap", gap: 8, justifyContent: "center", marginBottom: 28 }}>
        {["📊 Live Dashboard","⚡ Real-Time Stream","📈 Trend Analysis","🌡️ Response Heatmap","☁️ Word Cloud","🛠️ Survey Builder"].map(f => (
          <span key={f} style={{ background: "#f1f5f9", borderRadius: 20, padding: "4px 12px", fontSize: 11, color: "#475569", fontWeight: 600 }}>{f}</span>
        ))}
      </div>
      ...
      <div style={{ display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap" }}>
        ...
        <div style={{ fontSize: 11, color: "#94a3b8", alignSelf: "center" }}>
          or upload a CSV above to use real data
        </div>
      </div>

      <div style={{ marginTop: 20, fontSize: 10, color: "#cbd5e1" }}>
        Sample: {SAMPLE_DATA_METADATA.totalRows} fictional employees · {SAMPLE_DATA_METADATA.departments.length} departments
      </div>
    </div>
  );
}
...
export default function M9PulseSurvey() {
  ...
  useEffect(() => {
    ...
    setPulseOverride({
      orgPulse:   current.orgPulse,
      deptScores: current.deptData,
      week:       current.week,
      updatedAt:  Date.now(),
    });
  ...
  const startStream = useCallback(() => {
    ...
    streamRef.current = setInterval(() => {
      if (count < responses.length) {
        setLiveStream(p => [...p, { ...responses[count], id: Date.now() + count, time: new Date().toLocaleTimeString() }]);
        count++;
      } else {
  ...
  const handleAIIntervention = useCallback(async (dept) => {
    ...
    try {
      const text = await fetchAIIntervention({
        dept,
        prevScore:       Number(prevData?.pulseScore || 50),
        currScore:       Number(deptData?.pulseScore || 40),
        ...
        deptSize:        deptEmps.length,
        responseRate:    deptData?.responseRate || 0,
  ...
  // ── EMPTY STATE ──
  if (isEmpty) {
    return <M9EmptyState onShowSample={() => setShowSample(true)} />;
  }

  // ── SAFETY: if history is empty (shouldn't happen past isEmpty check) ──
  if (!current) return null;

  return (
    <div>
      {/* ── Sample Banner — sticky context for user ── */}
      {isUsingSample && (
        <SampleBanner onHide={() => setShowSample(false)} />
      )}
        ...
        <div style={{ marginBottom: 18 }}>
          <button
            ...
            style={{
              ...
              cursor: "pointer", marginBottom: showWarnings ? 8 : 0,
              color: "#dc2626", fontWeight: 700, fontSize: 13, transition: "all 0.2s",
            }}
          >
            <span style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ fontSize: 16 }}>🚨</span>
              <span>{earlyWarnings.length} Critical Burnout Warning{earlyWarnings.length > 1 ? "s" : ""}</span>
            </span>
          ...
          {showWarnings && earlyWarnings.map(w => (
            <div key={w.dept} style={{ background: "#fef2f2", borderRadius: 12, padding: "12px 18px", border: "1.5px solid #ef4444", marginBottom: 8, display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <div style={{ display: "flex", gap: 4 }}>
                {w.scores.map((s, i) => (
                  <div key={`${w.dept}-bar-${i}`} style={{ width: 6, height: Math.max(4, (s / 100) * 30), background: s < 40 ? "#ef4444" : s < 60 ? "#f59e0b" : "#22c55e", borderRadius: 2, alignSelf: "flex-end" }} />
                ))}
              </div>
            </div>
          ))}
      ...
      {activeTab === "stream" && (
        <div>
          {/* Info banner for simulated stream */}
          <div style={{ background: "#eff6ff", border: "1.5px solid #bfdbfe", borderRadius: 8, padding: "8px 14px", marginBottom: 12, fontSize: 11, color: "#1d4ed8", fontWeight: 600, display: "flex", alignItems: "center", gap: 8 }}>
            <span>🔵</span>
            {isUsingSample
              ? "Pulse data is simulated from demo employees. Upload your CSV then send real survey links to get live responses."
              : "Pulse data is simulated from your CSV. Real-time responses require M9 survey links sent to employees."}
          </div>
                ...
                {!streamActive && liveStream.length > 0 && (
                  <div style={{ background: "#f0fdf4", borderRadius: 10, padding: "10px 14px", border: "1px solid #bbf7d0", marginTop: 8 }}>
                    ...
                    <div style={{ fontSize: 10, color: "#64748b", marginTop: 3 }}>
                      Avg score: {(liveStream.reduce((s, r) => s + r.answer, 0) / liveStream.length).toFixed(1)}/10 — {liveStream.filter(r => r.answer < 4).length} low scores flagged
                    </div>
                  </div>
                )}
              </div>
                  ...
                  {depts.map((dept, di) => {
                    ...
                    return (
                      <tr key={dept} style={{ borderBottom: "1px solid #f8fafc", background: di % 2 === 0 ? "#fff" : "#fafafa" }}>
                        ...
                        {scores.map((s, i) => {
                          const sc     = s >= 70 ? "#22c55e" : s >= 50 ? "#f59e0b" : s >= 30 ? "#f97316" : "#ef4444";
                          const isLast = i === scores.length - 1;
                          return (
                            <td key={`w${i + 1}`} style={{ padding: "5px 4px", textAlign: "center" }}>
                              <div style={{ background: isLast ? sc : `${sc}22`, borderRadius: 5, padding: "3px 5px", fontSize: 11, fontWeight: isLast ? 800 : 600, color: isLast ? "#fff" : sc }}>
                                {s}
                              </div>
                            </td>
                          );
              ...
              {QUESTION_BANK.map(q => {
                ...
                return (
                  <div key={q.id} onClick={() => toggleQuestion(q.id)}
                    style={{ background: selected ? "#fffbeb" : "#f8fafc", borderRadius: 11, padding: "12px 14px", border: `2px solid ${selected ? "#f59e0b" : "#f1f5f9"}`, cursor: "pointer", transition: "all 0.15s" }}>
